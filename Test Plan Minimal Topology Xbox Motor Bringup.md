...
- robot config push and profile activation
...
  - CLI is used for DSL test import and validation, runtime group binding, robot push and activation, and command-driven runtime checks
...
    set "SPARKMAX/NEO 25".output = 0.20
...
    set "SPARKMAX/NEO 25".output = 0.15
...
### Step 4.3: Push and activate the config
...
config push src/main/deploy/bringup_system.json --activate test_minimal_25_9
...
- active robot profile becomes `test_minimal_25_9`

  

### Step 4.4: Confirm robot device/runtime state
...
### Step 4.5: Confirm the `motors` group reached the robot
...
### Step 7.3: Re-run both tests briefly
...
- config push and activation succeed
...
- the same behavior still works after restart/reconnect
