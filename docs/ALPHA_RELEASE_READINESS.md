...
2. Robot project builds and deploys via the normal GradleRIO workflow.
3. NetworkTables key contract is explicitly documented and stable.
...
1. Make device config + bringup test creation easy and reliable:
   ...
   - Provide a single validate+sync gate command that teams run every time after edits:
     - Validates canonical `data/bringup_system.json` (schema + semantic references).
     - Stamps `data_version`/`data_hash` when needed.
     - Writes deploy copy `src/main/deploy/bringup_system.json`.
2. Fix PC tool startup crash (done):
   - Regenerate/align CLI constants so `python -m tools.can_nt.can_nt_bridge --version` runs without crashing.
3. Remove or replace non-portable hardcoded scripts/paths:
   - `cli.bat` hardcodes a user-specific Python path.
   - Alpha requires one blessed entrypoint that works across machines.
4. Align Windows install script with actual tool needs:
   - `install_windows.ps1` should install dependencies that the docs and CLI expect (notably `prompt_toolkit`). (done)
5. Replace placeholder contract/setup docs:
...
- Tooling is available and works on Windows:
  - Bringup test wizard (`tools/bringup_test_wizard/...`) and/or template wizard (`tools/test_template_wizard/...`).
...
2. Enforce "read-only by default" on PC tool:
   - Any CAN transmit capability must be explicitly gated (e.g., `--tx-seq` requires a separate allow flag).
...
2. Fix the PC tool crash and confirm `--version` works.
3. Lock down Windows entrypoints (module invocation + optional wrapper).
4. Replace `docs/SETUP.md` and `docs/NT_CONTRACT.md` stubs with real content.
5. Add/refresh smoke checks and document exact pass/fail steps.
...
- Byte fingerprinting and candidate classification outputs published under `bringup/diag/can/...`.
- CI checks:
