...
| `show sources` vs `show workspace` vs `show session` | Overlapping status views. | Keep `show workspace`, deprecate others. |
...
| `ls` vs `show`, `val` vs `validate`, `prof` vs `profile`, `cfg` vs `configure terminal` | Hard aliases that duplicate grammar. | Prefer prefix-based abbreviation and remove hard aliases where possible. |
...
| `show session` | `show workspace` | Alias or remove. |
| `bindings show` | `show bindings` | Single entry point. |
| `can-mappings show` | `show can-mappings` | Single entry point. |
| `prof ...` | `profile ...` | Prefer prefix-based abbreviations. |
| `cfg` | `configure terminal` | Prefer prefix-based abbreviations. |
| `val ...` | `validate ...` | Prefer prefix-based abbreviations. |
| `ls` | `show` | Prefer prefix-based abbreviations. |
