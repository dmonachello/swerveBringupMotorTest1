...
Purpose: Store profiles, devices, controller devices, bindings, diagram, and bridgeConfig.byProfile in one file.
- Used by: CLI and topology editor.
- Contract: controller identity and ports live in `devices`; global command bindings live in the top-level `bindings` subtree.

Data Ownership
Purpose: Specify who owns which data.
- devices table + profiles + diagram: owned by topology editor or manual JSON edits.
- top-level `bindings`: owned by CLI and topology editor through the unified config.
- bridgeConfig.byProfile (groups, selectedDevice): owned by CLI and runtime tools.
...
Bridge CLI
- src/main/deploy/bringup_system.json: primary unified config (profiles + devices + bindings + groups + tests). Change likelihood: high. Read: default.
...
Bringup Control UI
- src/main/deploy/bringup_system.json: profile list + controller devices + labels for dropdowns/live view. Change likelihood: high. Read: default.
...
Topology Editor
- src/main/deploy/bringup_system.json: unified config with profiles + devices + bindings + diagram + bridgeConfig.byProfile. Change likelihood: high. Read: default.
...
- CLI loads bringup_system.json, edits groups/top-level bindings, and writes back to the same file.
...
- Optional: open topology editor and load the same file for layout.
