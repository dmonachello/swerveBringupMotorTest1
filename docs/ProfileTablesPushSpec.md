...
Enable explicit, TCP-based delivery of the full devices/profiles tables payload to the robot, with strict validation, staged apply, and no implicit apply.

## Scope

Includes:

- New TCP UI command for tables apply
- CLI commands to push tables payload and full config
...
- Tables payload: devices + profiles + default_profile from bringup_system.json
...
- If tables apply fails, stop
- If tables apply succeeds, run existing `import config <path>` for groups/bindings

Important:

- This is NOT atomic across both steps
- If import config fails, tables payload remains applied
...
- `registryJson` is raw JSON string for hash consistency (field name retained for wire compatibility)

## Apply Pipeline

Tables/config push uses a staged apply pipeline. Each stage must pass before the next stage begins.
...
- Replace in-memory tables payload atomically
...
Push tables payload and activate:
