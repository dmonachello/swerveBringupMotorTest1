...
- `src/main/deploy/bringup_system.json` is the shared configuration source for robot and PC tools.
- `schema_version` (4), `data_version` (`YYYY-MM-DD_HHMMSS`), and `data_hash` must be present.
- Profiles reference devices by label only; device identity lives in the devices table.
- Controllers are device entries, not a separate bindings registry.
- Global input bindings live under the top-level `bindings` subtree.
...
Topology Editor
- src/main/deploy/bringup_system.json: unified config with profiles + diagram + bindings + bridgeConfig.byProfile. Change likelihood: high. Read: default.
