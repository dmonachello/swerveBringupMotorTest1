...
/**
 * NAME
 *   RevReadFailureCooldown - Track recent REV read failures and gate repeated retries.
 *
 * DESCRIPTION
 *   Prevents high-rate retry loops from repeatedly provoking vendor warning/error
 *   output while a device remains unreachable. The caller decides how to surface
 *   the cooldown state to snapshots or sampled-signal readers.
 */
public final class RevReadFailureCooldown {
  private static final long DEFAULT_COOLDOWN_NS = 30_000_000_000L;
  private static final long NEVER_FAILED_AT_NS = Long.MIN_VALUE;
  private static final String DEFAULT_UNAVAILABLE_MESSAGE = "rev device read unavailable";
  ...
  /**
   * NAME
   *   RevReadFailureCooldown - Construct a cooldown tracker using the system monotonic clock.
   */
  public RevReadFailureCooldown() {
    this(System::nanoTime, DEFAULT_COOLDOWN_NS);
  }

  RevReadFailureCooldown(LongSupplier clock, long cooldownNs) {
