...
exec: show_exec | "configure" ws "terminal" | "connect" | "disconnect" | "add" ws ("next" | "all") | "reset" ws "zero-config" ( ws ("--yes" | "--clear-memory") )? ( ws ("--yes" | "--clear-memory") )? | "active" ws ("add" | "next" | ("show" ( ws "--json" )?)) | "run" ws "test" ( ws name )? | "tests" ws ("select" ws name | "toggle" | "run" | "run-all") | "profile" ws "device" ws "show-all" ws name | diagnose 
...
mappings_args: "show" ( ws ("manufacturers" | "device-types" | "device-type") )? ( ws show_flags )? | "manufacturer" ws ("set" ws number ws value_text | "delete" ws number | "no" ws number) | "device-type" ws ("set" ws number ws value_text | "delete" ws number | "no" ws number) | "load" ws path | "save" ws path | "validate" ( ws path )? 
