...
exec: show_exec | "configure" ws "terminal" | "connect" | "disconnect" | diagnose 
show_exec: "show" ws show_target ( ws show_flags )? 
show_target: "status" | "groups" | "group" ws name | "devices" | "device" ws name | "device" ws "registry" ws name | "bindings" | "selected-device" | "runtime-state" | "config" | "config" ws "local-raw" | "config" ws "dirty" | "profiles" | "profile" ( ws name )? | "tests" | "test" ws name | "message-level" 
show_source: "robot" | "local" | "both" 
show_flags: show_flag ( ws show_flag )* 
show_flag: show_source | "--json" | "--pretty" 
config: "group" ws name | "no" ws "group" ws name | "no" ws "device" ws name | "profile" ws name | "profile" ws "create" ws name | "selected-device" ws name | "selected-mode" ws ("on" | "off") | "merge" ws "config" ws path | "import" ws "config" ws path | "export" ws "runtime-groups" ws path | "export" ws "cli-script" ws path | "save" ws "config" ws path | "save" ws "local-config" ws path | "save" ws "profiles" ws path | "save" ws "unified-config" ws path | "rename" ws "device" ws name ws name | "device" ws name | "device" ws name ws "set" ws device_field ws value_text | "validate" ws "config" ( ws path )? | "show" ws show_target ( ws show_flags )? | diagnose | "bindings" ( ws bindings_args )? | "can-mappings" ( ws mappings_args )? | "tests" ( ws tests_args )? | "write" ws "tests" ws path | "test" ws ("set" ws name | "create" ws name | "delete" ws name | name) 
...
mappings_args: "show" ( ws ("manufacturers" | "device-types") )? ( ws show_flags )? | "manufacturer" ws ("set" ws number ws value_text | "delete" ws number | "no" ws number) | "device-type" ws ("set" ws number ws value_text | "delete" ws number | "no" ws number) | "load" ws path | "save" ws path | "validate" ( ws path )? 
tests_args: "templates" | "load" ws path | "merge" ws path | "load" ws "template" ws name | "save" | "clear" 
