...
exec: show_exec | "configure" ws "terminal" | "cfg" | "connect" | "disconnect" | "add" ws ("next" | "all") | "reset" ws "zero-config" ( ws ("--yes" | "--clear-memory") )? ( ws ("--yes" | "--clear-memory") )? | "active" ws ("add" | "next" | ("show" ( ws "--json" )?)) | "run" ws "test" ( ws name )? | "tests" ws ("select" ws name | "toggle" | "run" | "run-all") | "profile" ws "device" ws "show-all" ws name | "prof" ws "device" ws "show-all" ws name | diagnose 
...
device: ("show" | "ls") | ("show" | "ls") ws show_target ( ws show_flags )? | "set" ws device_field ws value_or_text | device_field ws value_or_text | "no" ws device_field | "delete" 
