...
exec: show_exec | "configure" ws "terminal" | "connect" | "disconnect" | "add" ws ("next" | "all") | "reset" ws "zero-config" ( ws ("--yes" | "--clear-memory") )? ( ws ("--yes" | "--clear-memory") )? | "active" ws ("add" | "next" | ("show" ( ws "--json" )?)) | "run" ws "test" ( ws name )? | "tests" ws ("select" ws name | "toggle" | "run" | "run-all" | tests_wait_args) | "profile" ws "export" ws name ws path ( ws "--install-robot" )? | "profile" ws "device" ws "show-all" ws name | diagnose
