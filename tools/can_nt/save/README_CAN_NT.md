...
```cmd
tools\can_nt\run_can_nt.cmd --verbose --print-summary-period 2
```
...
Config:
- Default settings live in `tools/can_nt/can_nt_config.json`.
...
- By default, `tools/can_nt/can_nt_config.json` enables CSV logging to `tools\can_nt\can_nt_log.csv`.
