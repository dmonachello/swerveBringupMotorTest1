...
Publishes:
  ...
  bringup/diag/api/<mfg>/<type>/<apiClass>/<apiIndex>/status
  bringup/diag/api/<mfg>/<type>/<apiClass>/<apiIndex>/ageSec
  bringup/diag/api/<mfg>/<type>/<apiClass>/<apiIndex>/msgCount
  bringup/diag/api/<mfg>/<type>/<apiClass>/<apiIndex>/lastSeen
...
def _get_config_path(argv: Optional[Iterable[str]]) -> str:
    ...
    if "--config" in args:
        ...
        if idx + 1 < len(args):
            return args[idx + 1]
    return "tools/can_nt/can_nt_config.json"
...
def main(argv: Optional[Iterable[str]] = None) -> int:
    ...
    parser.add_argument(
        "--log-raw-csv",
        default=config.get("log_raw_csv", ""),
        help="Path to raw-frame CSV log file (empty to disable)",
    )
    parser.add_argument(
        "--tag-events",
        action="store_true",
        help="Enable manual event tags from stdin (type a label + Enter)",
    )
    parser.add_argument(
        "--log-period",
        type=float,
    ...
    quick_done = False
    tag_queue: "queue.Queue[Tuple[float, str]]" = queue.Queue()
    last_tag_label = ""
    last_tag_time = 0.0
    ...
    def _write_raw_header(handle) -> None:
        handle.write(
            "timestamp,arb_id,mfg,type,apiClass,apiIndex,deviceId,dlc,dataHex,isError,tag,tagAgeSec\n"
        )
    ...
    def _write_raw_row(
        ...
        mfg, device_type, api_class, api_index, device_id = decoded
        data = getattr(msg, "data", b"") or b""
        dlc = len(data)
        data_hex = data.hex().upper()
        is_error = "1" if getattr(msg, "is_error_frame", False) else "0"
        tag_age = ""
        if tag_label and tag_time > 0:
            tag_age = f"{(timestamp - tag_time):.3f}"
        handle.write(
            f"{timestamp:.6f},0x{msg.arbitration_id:X},{mfg},{device_type},"
            f"{api_class},{api_index},{device_id},{dlc},{data_hex},{is_error},"
            f"{tag_label},{tag_age}\n"
        )

    api_last_seen: Dict[Tuple[int, int, int, int], float] = {}
    api_msg_count: Dict[Tuple[int, int, int, int], int] = {}

    if args.tag_events:
        print("Tag events enabled: type a label and press Enter to tag the log.")
        thread = threading.Thread(target=_tag_reader, daemon=True)
        thread.start()
    ...
    try:
        while True:
            ...
            while True:
                try:
                    tag_time, tag_label = tag_queue.get_nowait()
                except queue.Empty:
                    break
            ...
            if msg is not None:
                if getattr(msg, "is_error_frame", False):
                    ...
                    manufacturer, device_type, api_class, api_index, device_id = _decode_ext_id(
                        msg.arbitration_id
                    )
                    ...
                    api_key = (manufacturer, device_type, api_class, api_index)
                    api_last_seen[api_key] = now
                    api_msg_count[api_key] = api_msg_count.get(api_key, 0) + 1
                    total_frames += 1
                    summary_frames += 1
                    ...
                    if args.log_raw_csv:
                        if raw_log_handle is None:
                            raw_log_handle = open(args.log_raw_csv, "a", encoding="utf-8")
                        if not raw_header_written:
                            _write_raw_header(raw_log_handle)
                            raw_header_written = True
                        _write_raw_row(
                            raw_log_handle,
                            now,
                            msg,
                            (manufacturer, device_type, api_class, api_index, device_id),
                            last_tag_label,
                            last_tag_time,
                        )
            ...
            if now - last_publish >= args.publish_period:
                ...
                for api_key, ts in api_last_seen.items():
                    status, age, _missing = _format_status(ts, now, args.timeout)
                    base = f"api/{api_key[0]}/{api_key[1]}/{api_key[2]}/{api_key[3]}"
                    diag_table.getEntry(f"{base}/status").setString(status)
                    diag_table.getEntry(f"{base}/ageSec").setDouble(
                        -1.0 if age is None else float(age)
                    )
                    diag_table.getEntry(f"{base}/msgCount").setDouble(
                        float(api_msg_count.get(api_key, 0))
                    )
                    diag_table.getEntry(f"{base}/lastSeen").setDouble(
                        -1.0 if ts is None else float(ts)
                    )
