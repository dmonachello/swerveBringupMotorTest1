...
TOPOLOGY_INTERFACE = "TOPOLOGY"
TOPOLOGY_TYPE_CANNECT_DIRECT = "cannectDirect"
TOPOLOGY_TYPE_CANNECT_INJECT = "cannectInject"
...
TITLE_PROFILE_OVERWRITE_WARNING = "Profile Overwrite Warning"
MSG_PROFILE_OVERWRITE_MISMATCH = (
    "The current canvas is loaded from profile '{loaded}', but the save target is "
    "'{target}'.\n\nSaving now will overwrite '{target}' with the current canvas.\n\n"
    "Continue?"
)
...
class TopologyEditor(tk.Tk):
    ...
    def _on_profile_pick(self, _event: tk.Event) -> None:
        ...
        if not name:
            return
        previous_target = self.entry_profile.get().strip()
        self.entry_profile.delete(0, tk.END)
        self.entry_profile.insert(0, name)
        ...
        previous_loaded = self._profile_name
        self._load_profile_from_path(
            path,
            ask_profile=False,
            confirm_discard=True,
        ...
        if self._profile_name == previous_loaded:
            self.entry_profile.delete(0, tk.END)
            self.entry_profile.insert(0, previous_target)
            self._profile_pick_var.set(previous_loaded or previous_target)
    ...
    def _save_profile_to_path(
        ...
        loaded_profile_name = str(self._profile_name or EMPTY_STRING).strip()
        if (
            profile_name in profiles
            and loaded_profile_name
            and profile_name != loaded_profile_name
        ):
            confirmed = messagebox.askyesno(
                TITLE_PROFILE_OVERWRITE_WARNING,
                MSG_PROFILE_OVERWRITE_MISMATCH.format(
                    loaded=loaded_profile_name,
                    target=profile_name,
                ),
            )
            if not confirmed:
    ...
    def _is_infrastructure_device_entry(self, entry: Dict[str, object]) -> bool:
        ...
        """
        interface = str(entry.get(KEY_INTERFACE) or entry.get(KEY_INTERFACE_LEGACY) or EMPTY_STRING).strip()
        if interface.upper() == TOPOLOGY_INTERFACE:
            return False
    ...
    def _node_from_device_def(self, entry: Dict[str, object]) -> Optional[Node]:
        ...
        if self._is_dio_device_entry(entry):
            ...
            self._next_key += 1
            return node
        if self._is_topology_device_entry(entry):
            label = str(entry.get(KEY_LABEL, EMPTY_STRING)).strip()
            tags = self._normalize_tags(entry.get("tags", []))
            topology_type = str(entry.get(KEY_TYPE, EMPTY_STRING)).strip()
            category = (
                DIAGRAM_CATEGORY_CANNECT_INJECT
                if topology_type == TOPOLOGY_TYPE_CANNECT_INJECT
                else DIAGRAM_CATEGORY_CANNECT_DIRECT
            )
            node = Node(
                key=self._next_key,
                category=category,
                ...
                interface=TOPOLOGY_INTERFACE,
                vendor=str(entry.get(KEY_VENDOR, DIAGRAM_VENDOR_SWYFT)).strip() or DIAGRAM_VENDOR_SWYFT,
                device_type=str(entry.get(KEY_MODEL, DIAGRAM_DEVICE_WIRING)).strip() or DIAGRAM_DEVICE_WIRING,
                motor=str(entry.get(KEY_MODEL, DIAGRAM_DEVICE_WIRING)).strip() or DIAGRAM_DEVICE_WIRING,
                limits=None,
                terminator=None,
                x=0.0,
                row=0,
    ...
    def _is_topology_device_entry(self, entry: Dict[str, object]) -> bool:
        interface = str(entry.get(KEY_INTERFACE) or entry.get(KEY_INTERFACE_LEGACY) or EMPTY_STRING).strip()
        return interface.upper() == TOPOLOGY_INTERFACE
    ...
    def _vendor_for_device(self, entry: Dict[str, object]) -> str:
        interface = str(entry.get(KEY_INTERFACE) or entry.get(KEY_INTERFACE_LEGACY) or EMPTY_STRING).strip()
        if interface.upper() == TOPOLOGY_INTERFACE:
            return str(entry.get(KEY_VENDOR, DIAGRAM_VENDOR_SWYFT)).strip() or DIAGRAM_VENDOR_SWYFT
        manufacturer = entry.get("manufacturer")
    ...
    def _device_type_for_device(self, entry: Dict[str, object]) -> str:
        interface = str(entry.get(KEY_INTERFACE) or entry.get(KEY_INTERFACE_LEGACY) or EMPTY_STRING).strip()
        if interface.upper() == TOPOLOGY_INTERFACE:
            return str(entry.get(KEY_MODEL, DIAGRAM_DEVICE_WIRING)).strip() or DIAGRAM_DEVICE_WIRING
        manufacturer = entry.get("manufacturer")
        device_type = entry.get("deviceType")
        model = str(entry.get("model", "")).upper()
        if device_type == DEVTYPE_MOTOR:
    ...
    def _apply_node_updates_to_registry(self) -> None:
        ...
        for node in self._device_nodes():
            ...
            if isinstance(entry, dict):
                ...
                elif node.interface == TOPOLOGY_INTERFACE:
                    entry[KEY_INTERFACE] = TOPOLOGY_INTERFACE
                else:
                    entry[KEY_INTERFACE] = (
    ...
    def _device_entry_from_node(self, node: Node) -> Dict[str, object]:
        ...
        if node.interface == INTERFACE_DIO:
            ...
            if node.tags:
                entry["tags"] = list(node.tags)
            return entry
        if node.interface == TOPOLOGY_INTERFACE:
            entry = {
                KEY_LABEL: node.label,
                KEY_INTERFACE: TOPOLOGY_INTERFACE,
                KEY_TYPE: (
                    TOPOLOGY_TYPE_CANNECT_INJECT
                    if node.category == DIAGRAM_CATEGORY_CANNECT_INJECT
                    else TOPOLOGY_TYPE_CANNECT_DIRECT
                ),
                KEY_VENDOR: node.vendor or DIAGRAM_VENDOR_SWYFT,
                KEY_MODEL: node.motor or DIAGRAM_DEVICE_WIRING,
            }
            entry.pop(KEY_INTERFACE_LEGACY, None)
            if node.tags:
                entry["tags"] = list(node.tags)
    ...
    def _add_cannect_node(self, kind: str) -> None:
        """
        NAME
            _add_cannect_node - Create a topology-only SWYFT CANnect device node.
        """
        base_label = "CANnect Inject" if kind == "inject" else "CANnect Direct"
        label = self._next_cannect_label(base_label)
    ...
    def _next_cannect_label(self, base_label: str) -> str:
        """
        NAME
            _next_cannect_label - Build a unique CANnect label.
        """
        existing = {
            (n.label or "").strip().lower()
            for n in self._nodes
            if (n.category or "").lower() in (DIAGRAM_CATEGORY_CANNECT_DIRECT, DIAGRAM_CATEGORY_CANNECT_INJECT)
        }
        if base_label.strip().lower() not in existing:
            return base_label
        index = 2
        while True:
    ...
    def _on_list_edit_commit(self, _event: tk.Event) -> None:
        ...
        if column == "can_id":
            ...
            if not new_value:
                ...
                return
            for target in targets:
                old_label = str(target.label).strip()
                target.label = new_value
                if old_label and old_label != new_value:
                    self._rename_registry_label(old_label, new_value)
                    self._update_bridge_config_label_refs(old_label, new_value)
                    self._update_callout_target_labels(old_label, new_value)
        elif column == "tags":
    ...
    def _help_topics(self) -> Dict[str, str]:
        ...
        return {
            ...
            "Profiles & Export": (
                ...
                "\n"
                "- Save to Deploy writes directly to src/main/deploy/bringup_system.json.\n"
        ...
        if self._drag_free_y:
            for key, free_y in list(self._drag_free_y.items()):
                ...
                if ENABLE_CANNECT_FREE_FLOAT and self._is_cannect_cluster_member(node):
                    ...
                    node.bus_index = min(
                        max(node.bus_index, 0),
                        max(len(self._bus_offsets) - 1, 0),
                    )
                    if self._bus_offsets:
                        bus_offset = self._bus_offsets[node.bus_index]
                        node.row = 0 if free_y_for_bus < bus_offset else 1
                        node.free_y = free_y_for_bus - bus_offset
                    else:
                        node.row = 0 if free_y_for_bus < 0.0 else 1
                        node.free_y = free_y_for_bus
                    node.free_y_relative = True
