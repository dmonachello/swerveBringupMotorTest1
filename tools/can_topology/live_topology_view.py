...
def _category_for_device(entry: Dict[str, object]) -> str:
    interface = str(entry.get(KEY_INTERFACE, "")).upper()
    if interface == INTERFACE_TOPOLOGY:
        topology_type = str(entry.get(KEY_TYPE, "")).strip()
        if topology_type == TYPE_CANNECT_INJECT:
            return "cannect_inject"
        return "cannect_direct"
