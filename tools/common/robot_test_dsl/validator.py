...
from .compiler import CompileError, collect_compile_errors, compile_source, compile_source_best_effort
from .model import (
    BUILTIN_TIMER_NAME,
    CONDITION_MODE_BARE,
...
MESSAGE_UNKNOWN_SIGNAL_FMT = 'unknown signal on device "{device}": {signal}'
...
def validate_store(
    ...
    for name, entry in store.tests_by_name.items():
        if name in seen_names:
            result.errors.append(ValidationIssue("Duplicate test name.", test_name=name, field="source"))
            continue
        seen_names.add(name)
...
def _validate_entry(
    ...
    try:
        ...
        regenerated, compile_errors = compile_source_best_effort(name, entry.source)
        compile_errors = compile_errors or collect_compile_errors(entry.source)
        issues = compile_errors or [ex]
        for issue in issues:
            result.errors.append(
                ValidationIssue(
                    f"Compile error at line {issue.line_number}: {issue.message}",
                    test_name=name,
                    field="source",
                )
            )
        normalized_for_validation = entry.normalized if entry.normalized is not None else regenerated
        _validate_normalized(result, normalized_for_validation, device_catalog, signal_catalog)
        return
    ...
    if _normalized_json(regenerated) != _normalized_json(entry.normalized):
        result.errors.append(ValidationIssue("Source and normalized payload are out of sync.", test_name=name, field="source"))
...
def _validate_normalized(
    ...
    if not normalized.main.aborts and not normalized.main.successes and not normalized.main.untils:
        result.warnings.append(ValidationIssue("no until or success -> runs forever", test_name=normalized.name, field="main:"))
    if not normalized.main.untils and normalized.main.aborts and not normalized.main.successes:
        first_abort = normalized.main.aborts[0].text if normalized.main.aborts else "main:"
        result.warnings.append(
            ValidationIssue("only abort termination -> may never stop", test_name=normalized.name, field=first_abort)
        )
    if normalized.main.untils and not normalized.main.requires:
        first_until = normalized.main.untils[0].text if normalized.main.untils else "main:"
        result.warnings.append(
            ValidationIssue("until without require -> may pass without proof", test_name=normalized.name, field=first_until)
        )
    ...
    for bucket in (phase.aborts, phase.successes, phase.untils, phase.requires):
        for condition in bucket:
            ...
            if signal_meta is None:
                result.errors.append(
                    ValidationIssue(
                        MESSAGE_UNKNOWN_SIGNAL_FMT.format(
                            device=condition.reference.device,
                            signal=condition.reference.signal,
                        ),
                        test_name=test_name,
...
def _validate_set_statement(
    ...
    if target_meta is None:
        result.errors.append(
            ValidationIssue(
                MESSAGE_UNKNOWN_SIGNAL_FMT.format(
                    device=statement.target.device,
                    signal=statement.target.signal,
                ),
                test_name=test_name,
    ...
    if not target_meta.get("writable"):
        result.errors.append(ValidationIssue("set to read-only signal", test_name=test_name, field=statement.text))
        return
    if target_meta.get("writable") and target_meta.get("safeValue") is None and not target_meta.get("safeProvider"):
        result.errors.append(ValidationIssue("writable signal has no safe value", test_name=test_name, field=statement.text))
    ...
    if source_meta is None:
        result.errors.append(
            ValidationIssue(
                MESSAGE_UNKNOWN_SIGNAL_FMT.format(
                    device=statement.source.device,
                    signal=statement.source.signal,
                ),
                test_name=test_name,
                field=statement.text,
            )
...
def _validate_signal_writeable(
    ...
    if signal_meta is None:
        result.errors.append(
            ValidationIssue(
                MESSAGE_UNKNOWN_SIGNAL_FMT.format(device=device_name, signal=signal_name),
                test_name=test_name,
                field=field,
            )
        )
    ...
    if not signal_meta.get("writable"):
        result.errors.append(ValidationIssue("set to read-only signal", test_name=test_name, field=field))
    if signal_meta.get("writable") and signal_meta.get("safeValue") is None and not signal_meta.get("safeProvider"):
        result.errors.append(ValidationIssue("writable signal has no safe value", test_name=test_name, field=field))
