...
class ConfigSchemaStoreProfilesTests(unittest.TestCase):
    ...
    def test_topology_duplicate_node_key_error_names_profile(self) -> None:
        payload = self._base_payload()
        payload["topology"] = {
            "profiles": {
                "demo": {
                    "nodes": [
                        {"key": 7, "objectType": "device", "deviceRef": "lmtSw0"},
                        {"key": 7, "objectType": "junction", "label": "dup"},
                    ]
                }
            }
        }
        ...
        self.assertFalse(result.ok())
        messages = [issue.message for issue in result.errors()]
        self.assertIn("Profile 'demo' topology duplicate node key: 7.", messages)

    def test_topology_missing_edge_endpoint_error_names_profile(self) -> None:
        payload = self._base_payload()
        payload["topology"] = {
            "profiles": {
                "demo": {
                    "nodes": [
                        {"key": 1, "objectType": "device", "deviceRef": "lmtSw0"},
                    ],
                    "edges": [
                        {
                            "id": "edge_1",
        ...
        self.assertFalse(result.ok())
        messages = [issue.message for issue in result.errors()]
        self.assertIn("Profile 'demo' topology edge 'edge_1' references missing node endpoint.", messages)
