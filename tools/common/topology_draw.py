...
def draw_group_overlays(
    ...
    *,
    ...
    for idx, group in enumerate(groups):
        ...
        candidate_label_bounds = [
            (x0 + 4.0, y0 - 4.0 - label_h, x0 + 4.0 + label_w, y0 - 4.0),
            (x1 - 4.0 - label_w, y0 - 4.0 - label_h, x1 - 4.0, y0 - 4.0),
            (x0 + 4.0, y1 + 4.0, x0 + 4.0 + label_w, y1 + 4.0 + label_h),
            (x1 - 4.0 - label_w, y1 + 4.0, x1 - 4.0, y1 + 4.0 + label_h),
            (x0 - side_gap - label_w, y0, x0 - side_gap, y0 + label_h),
            (x1 + side_gap, y0, x1 + side_gap + label_w, y0 + label_h),
        ]

        current_label_bounds = candidate_label_bounds[0]
        for candidate in candidate_label_bounds:
            if any(_rects_overlap(candidate, other) for other in occupied_label_bounds):
                continue
            current_label_bounds = candidate
            break
        else:
            label_x0, label_y0, label_x1, label_y1 = candidate_label_bounds[0]
            current_label_bounds = (label_x0, label_y0, label_x1, label_y1)
            while any(_rects_overlap(current_label_bounds, other) for other in occupied_label_bounds):
                label_y1 = label_y0 - label_gap
                label_y0 = label_y1 - label_h
                current_label_bounds = (label_x0, label_y0, label_x1, label_y1)
        label_x0, label_y0, label_x1, label_y1 = current_label_bounds
        ...
        overlays.append(
            {
                "name": name,
                "bounds": (x0, y0, x1, y1),
                "label_bounds": current_label_bounds,
            }
        )
        occupied_label_bounds.append(current_label_bounds)
    return overlays
