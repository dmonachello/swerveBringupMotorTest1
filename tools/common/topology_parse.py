        ...
        if entry.get(KEY_NODE_TYPE) == NODE_TYPE_DEVICE:
            ...
            if device_ref:
                ...
                if isinstance(device, dict):
                    ...
                    if KEY_CATEGORY in device:
                        resolved_entry[KEY_CATEGORY] = device.get(KEY_CATEGORY)
                    resolved_entry[KEY_VENDOR] = device.get(KEY_VENDOR, EMPTY_STRING)
                    resolved_entry[KEY_MODEL] = device.get(KEY_MODEL, EMPTY_STRING)
                    resolved_entry[KEY_INTERFACE] = device.get(KEY_INTERFACE, EMPTY_STRING)
                    resolved_entry[KEY_DEVICE_TYPE] = device.get(KEY_DEVICE_TYPE, EMPTY_STRING)
                    if KEY_ID in device:
                        resolved_entry[KEY_ID] = device.get(KEY_ID)
...
def parse_diagram_nodes(diagram: Dict[str, object]) -> List[Dict[str, object]]:
    ...
    for entry in topology_nodes(diagram):
        ...
        if entry.get(KEY_NODE_TYPE) == NODE_TYPE_DEVICE:
            ...
            if KEY_INTERFACE in entry:
                compat[KEY_INTERFACE] = entry.get(KEY_INTERFACE)
            if KEY_DEVICE_TYPE in entry:
                compat[KEY_DEVICE_TYPE] = entry.get(KEY_DEVICE_TYPE)
            if KEY_ID in entry:
                compat[KEY_ID] = entry.get(KEY_ID)
        else:
